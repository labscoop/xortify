...
include_once XOOPS_ROOT_PATH . '/modules/xortify/class/auth/auth_rest_curl_provisionning.php';
...
class XortifyAuthRest_Curl extends XortifyAuth {
	...
	function authenticate($uname, $pwd = null) {
		$authenticated = false;
		$this->XortifyAuthRest_Curl($GLOBALS['xoopsDB']);
	...
	function network_disclaimer(){
		$this->XortifyAuthRest_Curl($GLOBALS['xoopsDB']);
		curl_setopt($this->curl_client, CURLOPT_URL, sprintf(XORTIFY_REST_API, 'xoops_network_disclaimer', http_build_query(array("username"=> $this->curl_xoops_username, "password"=> $this->curl_xoops_password))));
		$data = curl_exec($this->curl_client);
		curl_close($this->curl_client);
