 ...
 * @file:			auth_wgetxml_provisionning.php		
...
*/
class XortifyAuthRest_WgetxmlProvisionning {
	...
	function &getInstance(&$auth_instance)
	{
		...
		if (!isset($provis_instance)) {
			$provis_instance = new XortifyAuthRest_WgetxmlProvisionning($auth_instance);
		}
	 ...
	 */
	function XortifyAuthRest_WgetxmlProvisionning (&$auth_instance) {
	...
	function change_xml(&$xoopsUser, $datas, $uname, $pwd = null) {	
		$ret = false;
		$member_handler = $GLOBALS['xoops']->getHandler('member');
		$xoopsUser->setVar('pass', md5(stripslashes($pwd)));
		$tab_mapping = explode('|', $this->xml_field_mapping);
