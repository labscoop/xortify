...
class REST_CURLSERIALISEDXortifyExchange {
	...
	{
		$this->REST_CURLSERIALISEDXortifyExchange ();
	}
			...
			switch (XOOPS_CURLSERIAL_LIB){
				case "PHPCURLSERIAL":
					try {
						...
						curl_setopt($this->curl_client, CURLOPT_URL, sprintf(XORTIFY_REST_API, 'training', http_build_query(array(      "username"	=> 	$this->serial_xoops_username,
			...
			switch (XOOPS_CURLSERIAL_LIB){
				case "PHPCURLSERIAL":
					try {
						...
						curl_setopt($this->curl_client, CURLOPT_URL, sprintf(XORTIFY_REST_API, 'spoof'.$type, http_build_query(array(      "username"	=> 	$this->serial_xoops_username,
