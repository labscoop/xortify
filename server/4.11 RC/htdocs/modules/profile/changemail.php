...
if (!$GLOBALS['xoopsUser'] || $GLOBALS['xoopsConfigUser']['allow_chgmail'] != 1) {
    redirect_header(XOOPS_URL . "/modules/" . $GLOBALS['profileModule']->getVar('dirname', 'n') . "/", 2, _NOPERM);
}

if ($GLOBALS['profileModuleConfig']['htaccess']&&empty($_POST)) {
	$url = XOOPS_URL.'/'.$GLOBALS['profileModuleConfig']['baseurl'].'/changemail'.$GLOBALS['profileModuleConfig']['endofurl'];
...
if (!isset($_POST['submit']) || !isset($_POST['passwd'])) {
    ...
    if ($errors) {
        ...
        if ($member_handler->insertUser($GLOBALS['xoopsUser'])) {
            ...
            $xoopsMailer->useMail();
            $xoopsMailer->setTemplateDir($GLOBALS['profileModule']->getVar('dirname', 'n'));
