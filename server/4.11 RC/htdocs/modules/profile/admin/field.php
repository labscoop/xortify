...
case "save":
    ...
    if ( isset($_REQUEST['id'])  ) {
        ...
        $obj->setVar('field_moduleid', $GLOBALS['profileModule']->getVar('mid') );
    ...
    if ( $obj->getVar('field_config') && $obj->getVar('field_type') != 'validation' && $obj->getVar('field_type') != 'validation2' ) {
    ...
    } elseif ($obj->getVar('field_type') == 'validation' || $obj->getVar('field_type') == 'validation2') {
		$options=array();
		foreach($_POST['field_validation_options'] as $id => $rule_id) 
    ...
    if ( $profilefield_handler->insert($obj)  ) {
        ...
        if ( count($perm_arr) > 0 ) {
            foreach ($perm_arr as $perm ) {
                ...
                $criteria->add(new Criteria('gperm_modid', intval($GLOBALS['profileModule']->getVar('mid') )) );
                if ( isset($_REQUEST[$perm]) && is_array($_REQUEST[$perm])  ) {
                    ...
                    foreach ($_REQUEST[$perm] as $groupid ) {
                        ...
                        if ( !isset($groups[$groupid])  ) {
                            ...
                            $perm_obj->setVar('gperm_modid', $GLOBALS['profileModule']->getVar('mid') );
