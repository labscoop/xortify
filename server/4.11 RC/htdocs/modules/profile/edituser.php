...
if ($GLOBALS['profileModuleConfig']['htaccess']&&empty($_POST)) {
	$url = XOOPS_URL.'/'.$GLOBALS['profileModuleConfig']['baseurl'].'/edit,'.$op.$GLOBALS['profileModuleConfig']['endofurl'];
...
if ($op == 'save') {
    if (!$GLOBALS['xoopsSecurity']->check()) {
        redirect_header(XOOPS_URL . "/modules/" . $GLOBALS['profileModule']->getVar('dirname', 'n') . "/", 3, _US_NOEDITRIGHT . "<br />" . implode('<br />', $GLOBALS['xoopsSecurity']->getErrors() ));
        exit();
    }
    ...
    if (!empty($stop)) {
        ...
        $editable_fields = $gperm_handler->getItemIds('profile_edit', $GLOBALS['xoopsUser']->getGroups(), $GLOBALS['profileModule']->getVar('mid') );
        ...
        if (!$member_handler->insertUser($edituser)) {
            ...
            redirect_header(XOOPS_URL.'/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/userinfo.php?uid=' . $edituser->getVar('uid'), 2, _US_PROFUPDATED);
        }
    }
}
...
if ($op == 'avatarform') {
    ...
    if ($GLOBALS['xoopsConfigUser']['avatar_allow_upload'] == 1 && $GLOBALS['xoopsUser']->getVar('posts') >= $GLOBALS['xoopsConfigUser']['avatar_minposts']) {
        include_once $GLOBALS['xoops']->path('class/xoopsformloader.php');
        $form = new XoopsThemeForm(_US_UPLOADMYAVATAR, 'uploadavatar', XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/edituser.php', 'post', true);
    ...
    $form2 = new XoopsThemeForm(_US_CHOOSEAVT, 'chooseavatar', XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/edituser.php', 'post', true);
...
if ($op == 'avatarupload') {
    if (!$GLOBALS['xoopsSecurity']->check()) {
        redirect_header(XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/index.php', 3, _US_NOEDITRIGHT . "<br />" . implode('<br />', $GLOBALS['xoopsSecurity']->getErrors() ));
        exit;
    }
    ...
    if (empty($uid) || $GLOBALS['xoopsUser']->getVar('uid') != $uid) {
        redirect_header(XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/index.php', 3, _US_NOEDITRIGHT);
        exit();
    }
    if ($GLOBALS['xoopsConfigUser']['avatar_allow_upload'] == 1 && $GLOBALS['xoopsUser']->getVar('posts') >= $GLOBALS['xoopsConfigUser']['avatar_minposts']) {
        ...
        if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
            ...
            if ($uploader->upload()) {
                ...
                if (!$avt_handler->insert($avatar)) {
                    ...
                    redirect_header(XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/userinfo.php?t=' . time() . '&amp;uid=' . $GLOBALS['xoopsUser']->getVar('uid'), 3, _US_PROFUPDATED);
                }
            }
        }
        redirect_header(XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . "/edituser.php?op=avatarform", 3, $uploader->getErrors() );
    }
}
...
if ($op == 'avatarchoose') {
    ...
    redirect_header(XOOPS_URL . '/modules/' . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/userinfo.php?uid=' . $uid, 0, _US_PROFUPDATED);
}
