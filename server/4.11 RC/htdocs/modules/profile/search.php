...
switch ($op ) {
    ...
    case "search":
		...
		if ($GLOBALS['profileModuleConfig']['htaccess']&&empty($_POST)) {
			$url = XOOPS_URL.'/'.$GLOBALS['profileModuleConfig']['baseurl'].'/search,'.$op.$GLOBALS['profileModuleConfig']['endofurl'];
        ...
        $searchable_fields = $gperm_handler->getItemIds('profile_search', $groups, $GLOBALS['profileModule']->getVar('mid'));

        include_once $GLOBALS['xoops']->path('class/xoopsformloader.php');
        $searchform = new XoopsThemeForm("", "searchform", $_SERVER['REQUEST_URI'], "post");
    ...
    case "results":
        ...
        $xoBreadcrumbs[] = array('link' => XOOPS_URL . "/modules/" . $GLOBALS['profileModule']->getVar('dirname', 'n') . '/search.php', 'title' => _SEARCH);
