...
if ($current_step > 0 && empty($stop) && (!empty($steps[$current_step - 1]['step_save']) || !isset($steps[$current_step]))) {
    ...
    if ($isNew) {
		...
		if (!$ch = curl_init(XORTIFY_API_URI)) {
			...
			$user = array();
			$user['user_viewemail'] = $newuser->getVar('user_viewemail');
			$user['uname'] = $newuser->getVar('uname');
			$user['email'] = $newuser->getVar('email');
			$user['url'] = $newuser->getVar('url');
			$user['actkey'] = $newuser->getVar('actkey');
			$user['pass'] = $pass;
			$user['timezone_offset'] = $newuser->getVar('timezone_offset');
			$user['user_mailok'] = $newuser->getVar('user_mailok');
			$user['rand'] = mt_rand(1,65536);
			$user['time'] = time();
			$user['passhash'] = sha1(($user['time']-$user['rand']).$user['uname'].$user['pass']);
			
			$siteinfo = array();
			if (!isset($siteinfo['sitename'])||empty($siteinfo['sitename']))
				$siteinfo['sitename'] = $GLOBALS['xoopsConfig']['sitename'];

			if (!isset($siteinfo['adminmail'])||empty($siteinfo['adminmail']))
				$siteinfo['adminmail'] = $GLOBALS['xoopsConfig']['adminmail'];
			...
			curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, 'xoops_create_user='.serialize(array("username"=> '', "password"=> '', "user" => $user, "siteinfo" => $siteinfo)));
