...
if ($GLOBALS['profileModuleConfig']['htaccess']&&empty($_POST)) {
	...
	$thisUser = $user_handler->get($uid);
	$group = 0;
	foreach($thisUser->getGroups() as $groupid) {
		if (!is_object($group))
			$group = $group_handler->get($groupid);
	}
	if (!is_object($group))
		$group = $group_handler->get(XOOPS_GROUP_USERS);
	$url = XOOPS_URL.'/'.$GLOBALS['profileModuleConfig']['baseurl'].'/'.urlencode(str_replace(array(' ', '_', '/', '\\'), '-', strtolower($group->getVar('name')))).'/'.urlencode(str_replace(array(' ', '_', '/', '\\'), '-', strtolower($thisUser->getVar('uname')))).'/'.$uid.$GLOBALS['profileModuleConfig']['endofurl'];
...
if (is_object($GLOBALS['xoopsUser']) && $uid == $GLOBALS['xoopsUser']->getVar('uid')) {
    ...
    $GLOBALS['xoopsTpl']->assign('lang_applet', _PROFILE_MA_OAUTH);
