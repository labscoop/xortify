    ...
    function getEditElement($user, $profile)
    {
        ...
        switch ($this->getVar('field_type')  ) {
			...
			case "ip":
				...
				$element->addElement(new XoopsFormLabel('', sprintf(_PROFILE_MI_FIELD_IP, $ret['ip'])));
				$element->addElement(new XoopsFormHidden($name, $ret['ip']));      
				break;          
			case "proxy-ip":
	        	$GLOBALS['ProfileHasProxyIP'] = true;
				if (isset($_SERVER["HTTP_X_FORWARDED_FOR"])){ 
					$ip = (string)$_SERVER["HTTP_X_FORWARDED_FOR"]; 
					$ret['is_proxied'] = true;
				...
				$element = new XoopsFormElementTray($caption, '');			
				if ($ret['is_proxied']) {
					$element->addElement(new XoopsFormLabel('', sprintf(_PROFILE_MI_FIELD_PROXYIP, $ret['proxy-ip'])));
					$element->addElement(new XoopsFormHidden($name, $ret['proxy-ip']));
				}
				break;
			case "network-addy":
	        	$GLOBALS['ProfileHasNetworkAddy'] = true;
				if (isset($_SERVER["HTTP_X_FORWARDED_FOR"])){ 
					$ip = (string)$_SERVER["HTTP_X_FORWARDED_FOR"]; 
				...
				$element = new XoopsFormElementTray($caption, '');
				$element->addElement(new XoopsFormHidden($name, $ret['network-addy']));
				$element->addElement(new XoopsFormLabel('', sprintf(_PROFILE_MI_FIELD_NETWORKADDY, $ret['network-addy'])));
				break;				
			case "validate2":
            case "validation2":
            	$element = new XoopsFormText($caption, $name, 35, $this->getVar('field_maxlength'), '');
            	break;

			case "validate":
			case "validation":
			
				$GLOBALS['ProfileHasValidation'] = true;
				$element = new XoopsFormElementTray($caption, '');
				$element->addElement(new XoopsFormText('', $name, 35, $this->getVar('field_maxlength'), ''));

				$val_button = new XoopsFormButton('', 'validate', _PROFILE_VALIDATE);
				$val_button->setExtra('onclick="javascript:ValidateProfileForm(\''.$name.'\', \'regform\');"');
				$element->addElement($val_button);
				$element->addElement(new XoopsFormLabel('', '<div id="passnotice">&nbsp;</div>'));
				
				$GLOBALS['xoTheme']->addScript(XOOPS_URL.'/browse.php?Frameworks/jquery/jquery.js');
				$GLOBALS['xoTheme']->addScript(XOOPS_URL.'/modules/profile/js/jquery.json.validation.js');
				$GLOBALS['xoTheme']->addScript( null, array( 'type' => 'text/javascript' ), 'function ValidateProfileForm(element, regform) {
	var params = new Array();
	$.getJSON("'.XOOPS_URL.'/modules/profile/dojsonvalidate.php?passkey=' . md5(XOOPS_LICENSE_KEY.date('Ymdhi')) . '&value=" + $(\'#\' + element).val() + "&" + $(\'#\'+regform).serialize() + "&field_id=' . $this->getVar('field_id') . '", params, refreshformdesc);
}' );
