...
defined('XOOPS_ROOT_PATH') or die("XOOPS root path not defined");
	...
	function profile_oauth_approve($uri) {
		xoops_loadLanguage('forms', 'profile');
		
		$sform = new XoopsThemeForm(_FRM_PROFILE_OAUTH_APPROVE, 'oauth', $uri, 'post');
		


		$ele = array();
		$ele['op'] = new XoopsFormHidden('op', 'login');
		$ele['url'] = new XoopsFormHidden('url', $uri);
		$ele['submit'] = new XoopsFormElementTray('', 'submission');
		$ele['submit']->addElement(new XoopsFormButton('', 'approve', _FRM_PROFILE_APPROVE, 'submit'));
		$ele['submit']->addElement(new XoopsFormButton('', 'cancel', _FRM_PROFILE_CANCEL, 'submit'));
		
		$required = array('submit');
	...
	function profile_oauth_login($uri) {
		xoops_loadLanguage('forms', 'profile');
	
		$sform = new XoopsThemeForm(_FRM_PROFILE_OAUTH_LOGIN, 'oauth', $uri, 'post');

		$ele = array();
		$ele['op'] = new XoopsFormHidden('op', 'login');
		$ele['url'] = new XoopsFormHidden('url', $uri);
		$ele['uname'] = new XoopsFormText(_FRM_PROFILE_OAUTH_UNAME, 'uname', 25, 35);
		$ele['pass'] = new XoopsFormPassword(_FRM_PROFILE_OAUTH_PASS, 'pass', 25, 35);
		$ele['submit'] = new XoopsFormElementTray('', 'submission');
		
		$ele['submit']->addElement(new XoopsFormButton('', 'approve', _FRM_PROFILE_APPROVE, 'submit'));
		$ele['submit']->addElement(new XoopsFormButton('', 'cancel', _FRM_PROFILE_CANCEL, 'submit'));
	
		$required = array('uname', 'pass', 'submit');
	...
	function profile_get_email() {
		...
		$sform = new XoopsThemeForm(_FRM_PROFILE_FORM_GET_EMAIL, 'email', 'getemail.php', 'post');
		...
		$ele['email'] = new XoopsFormText(_FRM_PROFILE_FORM_EMAIL, 'email', 26,128);
		$ele['email']->setDescription(_FRM_PROFILE_FORM_DESC_EMAIL);
	 ...
	 * @return object
	 */
	function profile_getAppletForm(&$field, $action = false)
	{

		if ( $action === false ) {
			$action = $_SERVER['REQUEST_URI'];
		}
		$title = $field->isNew() ? sprintf(PROFILE_MN_ADD, PROFILE_MN_APPLET) : sprintf(PROFILE_MN_EDIT, _PROFILE_MN_APPLET);
	
		include_once $GLOBALS['xoops']->path('class/xoopsformloader.php');
		$form = new XoopsThemeForm($title, 'applet', $action, 'post', true);
	
		$form->addElement(new XoopsFormText(_PROFILE_MA_NAME, 'name', 35, 255, $field->getVar('name', 'e')));
		$form->addElement(new XoopsFormTextArea(_PROFILE_MA_DESCRIPTION, 'description', $field->getVar('description', 'e')));
		$form->addElement(new XoopsFormText(_PROFILE_MA_URI, 'uri', 35, 255, $field->getVar('uri', 'e')));
		$form->addElement(new XoopsFormText(_PROFILE_MA_ORG, 'org_name', 35, 255, $field->getVar('org_name', 'e')));
		$form->addElement(new XoopsFormText(_PROFILE_MA_ORGURI, 'org_uri', 35, 255, $field->getVar('org_uri', 'e')));
		$form->addElement(new XoopsFormFile(_PROFILE_MA_IMAGE, 'image', 2500000));
		$form->addElement(new XoopsFormHidden('profile_id',($field->isNew()?$GLOBALS['xoopsUser']->getVar('uid'):$field->getVar('profile_id', 'e'))));
		$client_id = $field->getVar('client_id', 'e');
		if ($field->isNew()) {
			include_once(dirname(dirname(__FILE__)).'/class/hash/hash.php');
			$hash = new XoopsHash();
			$client_id = $hash->getHash(microtime(true).print_r($_ENV, true), true); 
			$client_secret = $hash->getHash(microtime(true).dirname(__FILE__).XOOPS_DB_PASS, true);
		} else {
			$oauth_clients_handler = xoops_getmodulehandler('oauth_clients', 'profile');
			$criteria = new Criteria('client_id', $client_id);
			if (count($objects = $oauth_clients_handler->getObjects($criteria))>0)
				$client_secret = $objects[0]->getVar('client_secret');
		}
		$form->addElement(new XoopsFormLabel(_PROFILE_MA_CLIENT_ID,$client_id));
		$form->addElement(new XoopsFormLabel(_PROFILE_MA_CLIENT_SECRET,$client_secret));
		$form->addElement(new XoopsFormHidden('client_id',$client_id));
		$form->addElement(new XoopsFormHidden('client_secret',$client_secret));
		$form->addElement(new XoopsFormHidden('appid',$field->getVar('appid')));
