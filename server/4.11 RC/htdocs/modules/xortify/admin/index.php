 ...
 */
	...
	include(dirname(dirname(dirname(dirname(__FILE__)))). 'include' . DIRECTORY_SEPARATOR . 'cp_header.php');
	...
	ini_set("error_log" , XOOPS_ROOT_PATH."/uploads/errors.log.".md5(XOOPS_ROOT_PATH).".txt");
	...
	$GLOBALS['xoopsTpl']->assign('pathImageIcon', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['ImageIcon']);
	$GLOBALS['xoopsTpl']->assign('pathImageAdmin', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['ImageAdmin']);
	...
	case "signup":	
		...
		case "save":	
			...
			$xortifyAuth =& XortifyAuthFactory::getAuthConnection(false, $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['protocol']);
			...
			if ($stop!='') {
				...
				if ($disclaimer != _XOR_ADMIN_NONETWORKCOMM_DISCLAIMER) {
					...
					$GLOBALS['xoopsTpl']->assign('protocol', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['protocol']);
					switch($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['protocol']) {
						case 'curlserialised':
						case 'wgetserialised':
							$GLOBALS['xoopsTpl']->assign('port', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_uriserial']);
							break;
						case 'curl':
						case 'json':
							$GLOBALS['xoopsTpl']->assign('port', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_urijson']);
							break;
						case 'curlxml':
						case 'wgetxml':
							$GLOBALS['xoopsTpl']->assign('port', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_urixml']);
							break;
						case 'soap':
							$GLOBALS['xoopsTpl']->assign('port', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_urisoap']);
							break;
					}
				...
				if (!is_object($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['module']))
					$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['module'] = $moduleHandler->getByDirname('xortify');
				$configs = $configHandler->getConfigs(new Criteria('conf_modid', $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['module']->mid()) );
	...
	case "list":
		...
		if (!$bans = XoopsCache::read('xortify_bans_cache')) {
			...
			XoopsCache::write('xortify_bans_cache', $bans, $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_seconds']);
			XoopsCache::write('xortify_bans_cache_backup', $bans, ($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_seconds'] * 1.45));					
		}
		...
		if ($bans['bans']==0) {
			...
			switch($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['protocol']) {
				case 'curlserialised':
				case 'wgetserialised':
					$hostname = parse_url($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_uriserial'], PHP_URL_HOST);
					break;
				case 'curl':
				case 'json':
					$hostname = parse_url($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_urijson'], PHP_URL_HOST);
					break;
				case 'curlxml':
				case 'wgetxml':
					$hostname = parse_url($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_urixml'], PHP_URL_HOST);
					break;
				case 'soap':
					$hostname = parse_url($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_urisoap'], PHP_URL_HOST);
					break;
			}
