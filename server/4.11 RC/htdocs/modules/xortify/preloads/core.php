...
class XortifyCorePreload extends XoopsPreloadItem
{
	...
	function eventCoreIncludeCommonStart($args)
	{
		...
		$ip = xortify_getIP();
		if (isset($_SERVER['HTTP_REFERER'])&&$result = XoopsCache::read('xortify_'.strtolower(__FUNCTION__).'_'.md5($ip))) {
			if (strtolower(XOOPS_URL)==strtolower(substr($_SERVER['HTTP_REFERER'], 0, strlen(XOOPS_URL)))&&$result['time']<microtime(true)) {
				XoopsCache::write('xortify_'.strtolower(__FUNCTION__).'_'.md5($ip), array('time'=>microtime(true)+1800), 1800);
				return false;
			}
		}
		XoopsCache::write('xortify_'.strtolower(__FUNCTION__).'_'.md5($ip), array('time'=>microtime(true)+1800), 1800);
	...
	function eventCoreIncludeCommonEnd($args)
	{
		...
		if ((isset($result['when'])?(float)$result['when']:-microtime(true))+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_ip_cache']<=microtime(true)) {
			$result = array();
			...
			foreach(XortifyCorePreload::getFileListAsArray(XOOPS_VAR_PATH.'/caches/xoops_cache/', 'xortify') as $id => $file) {
				if (file_exists(XOOPS_VAR_PATH.'/caches/xoops_data/'.$file)&&!empty($file)) {
					if (@filectime(XOOPS_VAR_PATH.'/caches/xoops_data/'.$file)<time()-$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_ip_cache']) {
						$result['files']++;
			...
			XoopsCache::write('xortify_cleanup_last', $result, $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_ip_cache']*2);
		}
		
		if (isset($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['lid']))
			if ($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['lid']==0)
				unset($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]);
				
		if (strpos($_SERVER["PHP_SELF"], '/banned.php')>0) {
			return false;
		}
		
		if ((isset($_COOKIE['xortify']['lid'])&&$_COOKIE['xortify']['lid']!=0)||(isset($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['lid'])&&$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['lid']!=0)&&!strpos($_SERVER["PHP_SELF"], '/banned.php')) {
			header('Location: '.XOOPS_URL.'/banned.php');
			exit;
		} 

		// Detect if it is an internal refereer.
		if (isset($_SERVER['HTTP_REFERER'])&&(isset($GLOBALS['xortify'][XORTIFY_INSTANCE_KEY][__FUNCTION__]))) {
			if (strtolower(XOOPS_URL)==strtolower(substr($_SERVER['HTTP_REFERER'], 0, strlen(XOOPS_URL)))&&$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY][__FUNCTION__]<microtime(true)) {
				$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY][__FUNCTION__] = microtime(true)+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_ip_cache'];
				return false;
			}
		}
		$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY][__FUNCTION__] = microtime(true)+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['xortify_ip_cache'];
	    ...
	    if ((isset($result['time'])?(float)$result['time']:0)<=microtime(true)) {
			XoopsCache::write('xortify_core_include_common_end', array('time'=>microtime(true)+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']), $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']);
	...
	function eventCoreHeaderCacheEnd($args)
	{
		...
		if ((isset($result['time'])?(float)$result['time']:0)<=microtime(true)) {
			XoopsCache::write('xortify_core_header_cache_end', array('time'=>microtime(true)+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']), $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']);
			if (XortifyCorePreload::hasAPIUserPass()) { 		
	...
	function eventCoreFooterEnd($args)
	{
		...
		// Runs Security Preloader
		include_once XOOPS_ROOT_PATH.'/class/cache/xoopscache.php';
		$result = XoopsCache::read('xortify_core_footer_end');
		if ((isset($result['time'])?(float)$result['time']:0)<=microtime(true)) {
			XoopsCache::write('xortify_core_footer_end', array('time'=>microtime(true)+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']), $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']);
			if (XortifyCorePreload::hasAPIUserPass()) { 		
				include_once XOOPS_ROOT_PATH . ( '/modules/xortify/include/post.footer.end.php' );
			}
			XoopsCache::write('xortify_core_footer_end', array('time'=>microtime(true)), -1);
		}
	...
	function eventCoreHeaderAddmeta($args)
	{
		...
		if ((isset($result['time'])?(float)$result['time']:0)<=microtime(true)) {
			XoopsCache::write('xortify_core_header_add_meta', array('time'=>microtime(true)+$GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']), $GLOBALS['xortify'][XORTIFY_INSTANCE_KEY]['moduleConfig']['fault_delay']);
			if (XortifyCorePreload::hasAPIUserPass()) {	
