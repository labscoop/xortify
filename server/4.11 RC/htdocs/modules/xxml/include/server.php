...
if (!function_exists("xxml_xml2array")) {
	
	/*	Converts an string of XML to an Associative Array
	 *  xxml_xml2array($contents, $get_attributes=1, $priority = 'tag')
	 *
	 *  @param string $contents			XML Content
	 *  @param boolean $get_attributes	Get Element Attributes
	 *  @param string $priority			XML Tag with basis of priority
	 *  @return array
	 */
	function xxml_xml2array($contents, $get_attributes=1, $priority = 'tag') { 
 ...
 *  xxml_toXML($array, $name='array', $standalone=TRUE, $beginning=TRUE, $nested = 0)
 *
 ...
 *  @return string
 */
function xxml_toXML($array, $name='array', $standalone=TRUE, $beginning=TRUE, $nested = 0) 
{
		if ($beginning) {
			if ($standalone)
		...
		if (is_array($array)) {
			foreach ($array as $key=>$value) {
				...
				if (is_array($value)) {
					...
					$nested++;				
					$output .= xxml_toXML($value, $name, false, false, $nested);
					$nested--;
					$output .= str_repeat("\t", (1 * $nested)) . '</' . (is_string($key) ? $key : $name.'_' . $key) . '>' . "\n";
				} else {
					if (strlen($value)>0) {
...
if (isset($_POST)) {
	foreach($_POST as $field => $value) {
		if ($field==$plugin)
			$values[$field] = obj2array(new SimpleXMLElement(str_replace('\\"', '"', $value)));
		else
			$values[$field] = $value;
	}
}
...
echo xxml_toXML($ttlresult, $elename, true);
