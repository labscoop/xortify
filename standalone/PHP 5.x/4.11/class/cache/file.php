 ...
 *
 */
class CacheFile extends CacheEngine
{
    /**
    ...
    function init($settings = array())
    {
        ...
        $defaults = array('path' => dirname(dirname(dirname(__FILE__))) . '/data' , 'extension' => '.php' , 'prefix' => 'xoops_' , 'lock' => false , 'serialize' => false , 'duration' => 31556926);
        $this->settings = array_merge($defaults, $this->settings);
        if (!isset($this->file)) {
            Load::load('File');
            $this->file = File::getHandler('file', $this->settings['path'] . '/index.html', true);
        }
    ...
    function read($key)
    {
        ...
        if (!empty($data) && !empty($this->settings['serialize'])) {
            ...
            if (is_array($data)) {
                Load::load('Utility');
                $data = Utility::recursive('stripslashes', $data);
            }
