...
include_once _RUN_XORTIFY_ROOT_PATH . '/class/auth/auth_rest_curlserialised_provisionning.php';
...
class XortifyAuthRest_Curlserialised extends XortifyAuth {
	...
	function authenticate($uname, $pwd = null) {
		...
		$this->XortifyAuthRest_Curlserialised($_SESSION['xortify']['db'], sprintf(XORTIFY_REST_API, 'xoops_authentication', http_build_query(array("username"=> $this->curl_xoops_username, "password"=> $this->curl_xoops_password, "auth" => array('username' => $uname, "password" => $pwd, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pwd), "rand"=>$rnd)))));
	...
	function network_disclaimer(){
		$this->XortifyAuthRest_Curlserialised($_SESSION['xortify']['db'], sprintf(XORTIFY_REST_API, 'xoops_network_disclaimer', http_build_query(array("username"=> $this->curl_xoops_username, "password"=> $this->curl_xoops_password))));
		$data = curl_exec($this->curl_client);
		curl_close($this->curl_client);
		$result = unserialize($data);
		if ($result['ERRNUM']==1){
