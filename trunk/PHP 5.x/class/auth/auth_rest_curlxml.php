...
include_once _RUN_XORTIFY_ROOT_PATH . '/class/auth/auth_rest_curlxml_provisionning.php';
...
class XortifyAuthRest_Curlxml extends XortifyAuth {
	...
	function authenticate($uname, $pwd = null) {
		...
		$this->XortifyAuthRest_Curlxml($_SESSION['xortify']['db'], sprintf(XORTIFY_REST_API, 'xoops_authentication', http_build_query(array("username"=> $this->curl_xoops_username, "password"=> $this->curl_xoops_password, "auth" => array('username' => $uname, "password" => $pwd, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pwd), "rand"=>$rnd)))));
	...
	function validate($uname, $email, $pass, $vpass){
		$rnd = rand(-100000, 100000000);
		$this->XortifyAuthRest_Curlxml($_SESSION['xortify']['db'], sprintf(XORTIFY_REST_API, 'xoops_user_validate', http_build_query(array("username"=> $this->curl_xoops_username, "password"=> $this->curl_xoops_password, "validate" => array('uname' => $uname, "pass" => $pass, "vpass" => $vpass, "email" => $email, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pass), "rand"=>$rnd)))));
		$data = curl_exec($this->curl_client);
		curl_close($this->curl_client);
	...
	function network_disclaimer(){
		$this->XortifyAuthRest_Curlxml($_SESSION['xortify']['db'], sprintf(XORTIFY_REST_API, 'xoops_network_disclaimer', http_build_query(array("username"=> $this->curl_xoops_username, "password"=> $this->curl_xoops_password))));
		$data = curl_exec($this->curl_client);
		curl_close($this->curl_client);
