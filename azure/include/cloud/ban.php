...
if (!function_exists('ban_dolookup')) {
	...
	*  @return string
	*/
	function ban_dolookup($apikey,$ip){
...
function ban($username, $password, $bans, $comments) {
	...
	foreach ($bans as $key => $ban){
		$pass = true;
		if (!empty($ban['ip4'])) {
			$what2lookup = implode('.', array_reverse(explode('.',$ban['ip4'])));
			$octet = implode('.', ban_dolookup(_MI_SERVER_PROJECTHONEYPOT_BL_KEY, $what2lookup));
			if ((integer)$octet[3]==0)
				$pass = false;
		}
